// portfolio.js (Airtable 필드 수정 + DOM 자동생성 + 무한스크롤)
// 🔥 ROAS 필드 적용 + 안전한 DOM 처리

document.addEventListener('DOMContentLoaded', () => {
  console.log('📄 Portfolio.js starting with Fixed Airtable Fields...');
  ...
  function generateAllModals(records) {
    ...
    records.forEach((record, index) => {
      ...
      const roas = fields['ROAS'] || 'N/A';  // 🔥 ROI → ROAS 변경
    ...
    if (portfolioGrid) {
      portfolioGrid.innerHTML = `
        <div style="grid-column: 1 / -1; text-align: center; color: #ccc; padding: 4rem;">
          ...
          <p style="font-size: 0.9rem; color: #666; margin-top: 0.5rem;">Airtable 필드 매핑 완료 🎯</p>
        </div>
        <style>
          @keyframes spin {
  ...
  window.portfolioDebug = {
    ...
    showConnectionInfo: () => {
      const info = `
🔍 KAUZ Portfolio 연결 정보 (Airtable 필드 매핑 완료)
...
• 더 로딩 가능: ${hasMoreData ? '✅' : '❌'}
    ...
    loadFallbackData: async () => {
      console.log('🔄 Loading fallback data with ROAS field...');
      const fallbackData = getFallbackData();
      await initPortfolioWithData(fallbackData);
      
      alert(`📋 샘플 데이터를 표시했습니다.\n\n${fallbackData.length}개의 샘플 프로젝트 (ROAS 필드 포함)`);
    },
    ...
    checkFields: async () => {
      ...
      if (data.length > 0) {
        ...
        alert(`📋 Airtable 필드 구조:\n\n${fields.join('\n')}\n\n콘솔에서 자세한 정보를 확인하세요.`);
      } else {
        alert('❌ 데이터가 없어서 필드 구조를 확인할 수 없습니다.');
      }
    }
  };
  ...
  async function initPortfolio() {
    console.log('🚀 Initializing KAUZ Portfolio with Fixed Airtable Fields...');
    ...
    console.log('✅ Portfolio initialization complete with Fixed Airtable Fields');
    console.log(`🎯 Setup: ${portfolioData.length} total items, ROAS field mapped correctly`);
  }

  // ─── 🏁 최종 초기화 실행 ───
  initPortfolio();

  console.log('✅ Portfolio.js loaded - Airtable Fields Fixed (ROAS) + DOM Auto-generation');
